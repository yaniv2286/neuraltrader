...
class Backtester:
    ...
    def _load_legacy_processed_data(self) -> pd.DataFrame:
        """
        Legacy fallback method for loading processed data.
        Used when Modern Era loader fails.
        """
        logger.info("V8.0: Loading legacy processed data as fallback...")
        ...
        for chunk_start in range(0, len(ticker_files), chunk_size):
            ...
            logger.info(f"V8.0: Processing chunk {chunk_start//chunk_size + 1}: tickers {chunk_start}-{chunk_end-1}")
            ...
            for ticker_file in chunk_files:
                ...
                try:
                    ...
                    all_dfs.append(merged_df)
                    logger.info(f"V8.0: Loaded {ticker}: {len(merged_df)} rows with scores")
        ...
        logger.info(f"V8.0: Combining {len(all_dfs)} DataFrames...")
        combined_df = pd.concat(all_dfs, ignore_index=True, copy=False)
    ...
    def load_historical_data(self) -> pd.DataFrame:
        ...
        V8.0: Redirect to Modern Era data loader first, then fallback to processed data.
        
        Returns:
            Combined DataFrame with historical data
        """
        logger.info("V8.0: load_historical_data() redirecting to Modern Era loader...")
