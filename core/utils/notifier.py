...
class EmailNotifier:
        ...
        try:
            ...
            if log_file_path and os.path.exists(log_file_path):
                try:
                    ...
                    logger.info(f"[OK] Log file attached: {log_file_path}")
                    
                except Exception as e:
                    logger.error(f"[FAIL] Error attaching log file: {e}")
            else:
                if log_file_path:
            ...
            logger.info("[OK] Message accepted by Gmail server")
            logger.info(f"[OK] Email sent successfully to {to_email}")
            return True
            
        except Exception as e:
            logger.error(f"[FAIL] Full error details: {type(e).__name__}: {e}")
            return False
        ...
        try:
            ...
            if attachment_path and os.path.exists(attachment_path):
                try:
                    ...
                    logger.info(f"[OK] File attached: {attachment_path}")

                except Exception as e:
                    logger.error(f"[FAIL] Error attaching file: {e}")
            ...
            logger.error(f"[FAIL] Error sending HTML email: {e}")
            return False
    ...
    def send_daily_brief(self, account_info: Dict, current_positions: List[Dict], 
        ...
        try:
            ...
            # Build subject line
            now_str = now_eastern.strftime('%Y-%m-%d')
            subject = f"[OK] NeuralTrader Daily Brief - {now_str}"
            ...
            if log_file_path:
                if os.path.exists(log_file_path):
                    try:
                        ...
                        logger.info(f"[OK] Log file attached: {filename}")
                    except Exception as attach_error:
                        logger.error(f"[FAIL] Failed to attach log file: {attach_error}")
...
"""
                        ...
                        try:
                            ...
                            logger.warning("[WARN] Created fallback attachment for missing log file")
                        except Exception as fallback_error:
                            logger.error(f"[FAIL] Failed to create fallback attachment: {fallback_error}")
                else:
                    logger.error(f"[FAIL] Log file not found: {log_file_path}")
                    print(f"FAILED TO ATTACH LOG FILE: File not found - {log_file_path}")
...
"""
            ...
            if attachment_file and os.path.exists(attachment_file):
                try:
                    ...
                    logger.info(f"[OK] Additional file attached: {filename}")
                except Exception as attach_error:
                    logger.error(f"[FAIL] Failed to attach additional file: {attach_error}")
                    print(f"FAILED TO ATTACH ADDITIONAL FILE: {attach_error}")
            elif attachment_file:
                logger.error(f"[FAIL] Additional file not found: {attachment_file}")
