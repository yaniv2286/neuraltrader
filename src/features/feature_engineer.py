...
def add_momentum_features(df: pd.DataFrame, window: int = 10) -> pd.DataFrame:
    ...
    df['momentum_pct'] = df['close'].pct_change(window)
...
def add_price_action_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Engulfing patterns
    bullish_engulfing = (df['close'].shift(1) < df['open'].shift(1)) & (df['close'] > df['open'])
    bearish_engulfing = (df['close'].shift(1) > df['open'].shift(1)) & (df['close'] < df['open'])
...
def add_lag_features(df: pd.DataFrame, lag: int = 1) -> pd.DataFrame:
    ...
    df['return_lag'] = df['close'].pct_change().shift(lag)
    df['volume_lag'] = df['volume'].shift(lag)
    df['rsi_lag'] = df.get('rsi', pd.Series()).shift(lag)
