...
class RiskManager:
    ...
    def check_portfolio_circuit_breaker(self, current_value: float, peak_value: float, 
        ...
        try:
            ...
            # Check if drawdown exceeds 12% threshold
            if drawdown > 0.12:  # 12% drawdown threshold
                # Trigger circuit breaker - 8 calendar days (covers ~6 trading days)
                new_cooldown = current_date + timedelta(days=8)
                new_cooldown_str = new_cooldown.isoformat()
                
                self.logger.error(f"🚨 CIRCUIT BREAKER TRIGGERED! Drawdown: {drawdown_pct:.2f}%")
                self.logger.error(f"🚨 Trading HALTED until: {new_cooldown_str}")
                
                return True, new_cooldown_str
            else:
                # Safe to trade
                self.logger.info(f"✅ Circuit breaker SAFE: Drawdown {drawdown_pct:.2f}% < 12% threshold")
                return False, None
    ...
    def calculate_position_size_risk(self, ticker: str, entry_price: float, 
                                   ...
                                   max_risk_pct: float = 0.01) -> int:
        """
        Calculate position size based on risk per trade (1% of portfolio by default).
