"""
NeuralTrader Ensemble Training - Tiingo Integration
===================================================

Train XGBoost, LightGBM, and RandomForest models using Tiingo data.
Uses adjClose for dividend/split adjusted calculations.
Implements 50-stock batch processing for API compliance.
"""
...
class TiingoEnsembleTrainer:
    """
    Ensemble trainer using Tiingo data source
    """
    ...
    def __init__(self):
        ...
        print("🚀 Tiingo Ensemble Trainer initialized")
        print(f"📁 Models directory: {self.models_dir}")
        print(f"📊 Batch size: {self.tiingo_manager.batch_size}")
        print()
    
    def load_training_data(self, max_tickers: int = 50) -> pd.DataFrame:
        """
        Load training data from Tiingo
        ...
        """
        print(f"[STEP 1] Loading data from Tiingo...")
        
        # Get S&P 100 data
        tickers = self.tiingo_manager.sp100_tickers[:max_tickers]
        print(f"[DATA] Fetching data for {len(tickers)} tickers")
        ...
        data_dict = self.tiingo_manager.get_batch_data(tickers, start_date, end_date)
        
        if not data_dict:
            raise ValueError("No data fetched from Tiingo")
        
        print(f"[OK] Loaded data for {len(data_dict)} tickers")
        ...
        for ticker, df in data_dict.items():
            try:
                ...
                if len(df) < 252:  # At least 1 year of data
                    print(f"[SKIP] {ticker}: Insufficient data ({len(df)} days)")
                    continue
                
                # Use adjClose for all calculations
                if 'Adj Close' not in df.columns:
                    print(f"[SKIP] {ticker}: No adjClose data")
                    continue
    ...
    def generate_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for ticker in df['ticker'].unique():
            ...
            try:
                ...
                features_df = self.feature_engineer.generate_features(ticker_df)
                
                if features_df is not None and len(features_df) > 0:
                    # Add target back
                    features_df['target'] = ticker_df['target'].values
    ...
    def save_models(self, models: Dict, scaler, feature_cols: List[str]) -> None:
        ...
        metadata = {
            ...
            'data_source': 'Tiingo',
            'batch_size': self.tiingo_manager.batch_size,
            'universe_size': len(self.tiingo_manager.sp100_tickers)
        }
    ...
    def run_training(self, max_tickers: int = 50) -> bool:
        ...
        try:
            print("=" * 80)
            print("NeuralTrader Tiingo Ensemble Training")
            print("=" * 80)
            ...
            print()
            print("=" * 80)
            print("[SUCCESS] Tiingo Ensemble Training Complete")
            print("=" * 80)
