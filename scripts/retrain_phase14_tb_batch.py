#!/usr/bin/env python3
"""
Phase 14 - Triple-Barrier Multi-Class Retraining (Batch HGB)
===========================================================
Same as retrain_phase14_tb.py but uses batch training for HGB to avoid memory errors.
"""
import os
import sys
import json
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(PROJECT_ROOT / 'logs' / 'retrain_phase14_batch.log', mode='w', encoding='ascii'),
    ]
)
...
# Triple-Barrier params (Config B - realistic/actionable)
TP_PCT = 0.05
STOP_ATR_MULT = 1.5
STOP_MIN = 0.03
STOP_MAX = 0.08
...
# Raw OHLCV columns to EXCLUDE from features (non-stationary)
RAW_OHLCV_COLS = {
...
HGB_PARAMS = dict(
    ...
    max_bins=32,
    early_stopping=False,  # Disable to avoid train_test_split memory
)
...
def build_features(df: pd.DataFrame, spy_close: pd.Series) -> pd.DataFrame:
    """Build features: FeatureEngineer + 4 extras, then DROP raw OHLCV (non-stationary)."""
    ...
    # DROP raw OHLCV columns (non-stationary, cause model to not generalize)
    cols_to_drop = [c for c in feats.columns if c in RAW_OHLCV_COLS]
    if cols_to_drop:
...
def train_model_batch(X_tr, y_tr, X_te, y_te, model_name, params):
    """Train HGB on a stratified subsample (warm_start batching is broken)."""
    logger.info(f'[{model_name.upper()}] Training with stratified subsample...')

    MAX_ROWS = 2_000_000  # 2M rows fits comfortably in memory for HGB
    ...
    if len(X_tr) > MAX_ROWS:
        ...
        X_sub = X_tr[idx]
        y_sub = y_tr[idx]
        logger.info(f'[{model_name.upper()}] Subsampled {len(X_tr):,} -> {len(X_sub):,} rows (stratified)')
    else:
        X_sub = X_tr
        y_sub = y_tr

    # Inverse-frequency sample weights
    class_counts = np.bincount(y_sub, minlength=3)
    class_weights = len(y_sub) / (3.0 * np.maximum(class_counts, 1))
    sample_w = class_weights[y_sub]

    model = HistGradientBoostingClassifier(**params)
    model.fit(X_sub, y_sub, sample_weight=sample_w)

    # Verify 3-class output
    classes = getattr(model, 'classes_', [])
    logger.info(f'[{model_name.upper()}] Model classes: {classes}')
    if len(classes) < 3:
        logger.warning(f'[{model_name.upper()}] Only {len(classes)} classes detected! Expected 3.')
    ...
    # Precision@0.65 per class (safe for any number of columns)
...
def train_model(X_tr, y_tr, X_te, y_te, model_name, params):
    ...
    class_weights = len(y_tr) / (3.0 * np.maximum(class_counts, 1))
    ...
    if model_name == 'xgboost':
        ...
        return train_model_batch(X_tr, y_tr, X_te, y_te, model_name, params)
    else:
    ...
    logger.info(f'[{model_name.upper()}] Model classes: {classes}')
    if len(classes) < 3:
        logger.error(f'[{model_name.upper()}] CRITICAL: Only {len(classes)} classes! Expected 3 (0,1,2).')
...
def main():
    ...
    t0 = datetime.now()
    logger.info('=== PHASE 14 TRIPLE-BARRIER RETRAINING (BATCH HGB) ===')
    ...
    # Remap -1 -> 2 for sklearn (NEUTRAL=0, LONG_WIN=1, SHORT_WIN=2)
    y_xgb = np.where(y == -1, 2, y).astype(np.int8)
    unique_classes = np.unique(y_xgb)
    logger.info(f'[LABELS] Remapped classes: {unique_classes}  (expect [0, 1, 2])')
    if len(unique_classes) < 3:
        raise RuntimeError(f'[FATAL] Only {len(unique_classes)} classes after remap: {unique_classes}. Need 3 for 3-class model.')
    ...
    metadata = {
        'phase': 'Phase15_TB_3class',
        'label_type': 'triple_barrier_3class',
        'classes': '0=NEUTRAL, 1=LONG_WIN, 2=SHORT_WIN',
